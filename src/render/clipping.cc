...
static void clip_plane(Vec3 plane_point, Vec3 plane_normal, Triangle &triangle, std::list<Triangle> &triangles_new)
{
    ...
    if (inside.size() == 1)
    {
        ...
        t.color = sf::Color::Green;//triangle.color;
        t.v0 = inside.front();
    ...
    if (inside.size() == 2)
    {
        ...
        t1.color = sf::Color::Blue;//triangle.color;
        t2.color = sf::Color::Red;//triangle.color;
    ...
    for (Triangle &triangle : triangles)
    {
        ...
        for (int plane = 0; plane < 4; plane++)
        {
            ...
            while (size > 0)
            {
                ...
                switch (plane)
                {
                case 0: clip_plane(Vec3(w-2,   0,   0), Vec3(-1,  0, 0), test, triangles_new); break;  // right
                case 1: clip_plane(Vec3(  1,   0,   0), Vec3( 1,  0, 0), test, triangles_new); break;  // left
                case 2: clip_plane(Vec3(  0, h-2,   0), Vec3( 0, -1, 0), test, triangles_new); break;  // top
                case 3: clip_plane(Vec3(  0,   1,   0), Vec3( 0,  1, 0), test, triangles_new); break;  // bottom
                }
